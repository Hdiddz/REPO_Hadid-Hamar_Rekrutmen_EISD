...
@section('content')
    ...
    <form method="GET" action="{{ route('jobs.index') }}" class="mb-6 sm:mb-7 rounded-2xl border border-slate-200 bg-white p-3.5 sm:p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900" data-reveal>
        <div class="grid gap-3 sm:grid-cols-2 md:grid-cols-[1.4fr_1fr_1fr_auto]">
            <div class="sm:col-span-2 md:col-span-1">
                <div class="flex items-center justify-between mb-1.5">
                    <label for="q" class="block text-xs font-bold text-slate-600 dark:text-slate-300">Posisi atau lokasi</label>
                    <button type="button" onclick="document.getElementById('mobileFiltersPanel').classList.toggle('hidden')" class="md:hidden inline-flex items-center gap-1 text-[11px] font-bold text-brand-700 dark:text-brand-400 hover:underline transition cursor-pointer">
                        <span class="material-symbols-outlined text-[15px]">tune</span>
                        <span>Filter Kategori & Skill</span>
                        @if(request()->hasAny(['category', 'skill']))
            ...
            <div id="mobileFiltersPanel" class="{{ request()->hasAny(['category', 'skill']) ? 'contents' : 'hidden md:contents' }}">
                <div>
            ...
            <div class="flex items-end gap-2 sm:col-span-2 md:col-span-1">
                <button type="submit" class="portal-button-primary min-h-11 flex-1 md:flex-none justify-center">
                    <span class="material-symbols-outlined text-[19px]">tune</span>
                    <span>Terapkan</span>
                </button>
                @if(request()->hasAny(['q','category','skill']))
    ...
    <div class="grid gap-3.5 sm:gap-4 md:grid-cols-2 xl:grid-cols-3">
        @forelse ($jobs as $job)
            <article class="group relative flex min-h-56 sm:min-h-64 flex-col rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 transition hover:-translate-y-0.5 hover:border-brand-300 hover:shadow-xl dark:border-slate-800 dark:bg-slate-900" data-reveal>
                @if($job->cover_image)
                    <div class="relative -mx-4 -mt-4 sm:-mx-5 sm:-mt-5 mb-3.5 h-36 overflow-hidden rounded-t-2xl bg-slate-900">
                        <img src="{{ $job->cover_image_url }}" alt="{{ $job->title }}" class="h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy">
