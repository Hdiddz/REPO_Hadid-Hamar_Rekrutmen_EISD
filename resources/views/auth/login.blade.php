...
@section('content')
    <div class="mx-auto max-w-md" data-reveal>
        ...
        <form id="loginForm" action="{{ route('login') }}" method="POST" class="space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:space-y-5 sm:rounded-3xl sm:p-7 sm:shadow-[0_24px_70px_-40px_rgba(15,23,42,0.45)] dark:border-slate-800 dark:bg-slate-900">
            ...
            <label class="flex cursor-pointer items-center gap-2.5 text-xs sm:text-sm text-slate-600 dark:text-slate-300"><input type="checkbox" name="remember" value="1" @checked(old('remember')) class="h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500">Ingat saya di perangkat ini</label>
