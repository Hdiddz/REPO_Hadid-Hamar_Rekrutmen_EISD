...
@section('portal_role_label', 'Administrator SDG 8')
@section('portal_user_initials', 'AD')
@section('portal_user_name', auth()->user()?->name ?? 'Tim Pengawas SDG 8')
@section('portal_user_role', 'Administrator')
@section('portal_user_context', 'Pengawasan dan tata kelola sistem')
@section('portal_context', 'Pusat tata kelola SDG 8')
...
@section('portal_mobile_navigation')
    ...
    <a href="{{ route('admin.master') }}" class="flex min-h-11 items-center gap-3 rounded-xl px-3 py-2 text-sm font-semibold {{ request()->routeIs('admin.master') ? 'bg-brand-50 text-brand-800 dark:bg-brand-950 dark:text-brand-200' : 'text-slate-700 dark:text-slate-200' }}"><span class="material-symbols-outlined">database</span>Master data</a>
